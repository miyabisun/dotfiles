...
2. spike / polish の内側から呼ばれた場合、その段階を継承する
   ...
   `refactor` / `slim` の内側では `polish` の議論軸を使う。
   slimでは意図した仕様削減の影響を、残す用途・利用習慣・互換性・復旧の容易さで比較する。
...
- spike / polish / refactor / slim の内側: **未解決の product / UX 選択が実装結果を変えるとき**。
...
user への応答は**結論を先に置く**。議論したこと自体を成果にしない。
