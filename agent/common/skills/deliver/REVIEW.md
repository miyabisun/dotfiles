...
Ponytailによる簡素化、コードの書式、lint、テストの再実行は主担当が持ち、ここでは重ねない。
UIの意匠とDESIGN.mdへの適合は製品要件として確認する。既存の画面と検証証拠を使い、別のUI承認は設けない。
指摘は担当が修正する。既存の授権を聞き直さず、文書の補足や可逆な修正にユーザーの署名を求めない。
