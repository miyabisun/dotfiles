...
報告無き仕事は未完了である。receipt (verbatim の出力・`path:line`・verdict・
召喚回数) はレビュワーと knowledge のための証拠で、削らない・要約しない・
隠さない。ただし **user が最初に読むのは証拠ではない**。報告は次の順で書く:

1. **森** — 何が変わったか、何ができるようになったか。日本語の文で書く。
   識別子・path・key 名は、文で説明したあとに括弧で添える。
2. **決めてほしいこと** — user にしか決められない決断を 1 つずつ名指しする。
   無ければ「無し」。
3. **木** — receipt。verbatim の出力と `path:line` はここに置く。上の 2 つに
   混ぜない。

木を先に出す報告、森を省いて木だけの報告、木を隠した報告は、いずれも
P19 を満たさない。
