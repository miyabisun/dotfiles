...
description: >-
  ...
  ゴールは存在しない)、formatter/linter は機械的に実行し、証拠の誠実さと
...
3. **TDD で作る**: この手順は**実行可能コードを作るとき**のものである。
   ...
   続ける。ここで止まってよい条件は
   「[停止の規範](../deliver/CONTRACT.md#停止の規範)」だけが定める。
...
止まってよい条件は CONTRACT.md の
「[停止の規範](../deliver/CONTRACT.md#停止の規範)」が正本である。
version を理由に段階を切り替えることもしない。
