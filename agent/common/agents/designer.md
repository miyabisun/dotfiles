...
2. テンプレートに従う宣言があれば原本を読む。パスは次のいずれか（install で同じ `agent/common/designs` にリンクされる）:
   - `~/.claude/designs/[template]/DESIGN.md`
   - `~/.cursor/designs/[template]/DESIGN.md`
   共有ルール（クロウム・タイポ・スペーシング・角丸・アイコン・フォーカスリング）は原本が正、データ色・ドメインコンポーネントはプロジェクト側が正。
3. プロジェクトに DESIGN.md が無ければ上記 templates から `docs/DESIGN.md` を生成する（従う宣言 + プロジェクト固有分のみ）。どれを使うか判断できなければ clarify。
