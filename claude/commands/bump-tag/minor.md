---
description: バージョンを minor bump (x.Y.z の Y+1、patch は 0 リセット) してタグを打ち、リリース CI を発火させる
allowed-tools: Bash(git *), Bash(cargo *), Bash(gh *), Read, Edit
---

# bump-tag: minor

現在のリポジトリのバージョンを **minor** bump (`x.Y.z` → `x.(Y+1).0`) し、コミット・タグ・push まで一気に行う。
...
- **minor bump**: `x.Y.z` → `x.(Y+1).0` (patch は 0 にリセット)
